  ...
  return (
    <Link
      ...
      className={`block p-6 rounded-xl border-2 transition-all duration-200 ${
        locked
          ? 'border-sand-200 bg-sand-50 opacity-75 hover:opacity-100'
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-sand-900 mb-2">{post.title}</h3>
      {post.excerpt && (
        <p className="text-sand-600 line-clamp-2">{post.excerpt}</p>
      )}
    </Link>
  );
}
