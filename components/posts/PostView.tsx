    ...
    <article className="max-w-3xl mx-auto">
      ...
      <header className="mb-12">
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <span className="text-sand-400">•</span>
          <span className="text-sm text-sand-500">{readTime} min read</span>
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-sand-900 mb-6 leading-tight tracking-tight">
          {post.title}
        </h1>
        {post.tagline && (
          <p className="text-xl md:text-2xl text-breezy-600 font-medium mb-6 leading-relaxed">{post.tagline}</p>
        )}
        {post.excerpt && (
          <p className="text-lg text-sand-600 leading-relaxed border-l-4 border-breezy-200 pl-4 italic">{post.excerpt}</p>
        )}
      </header>

      {/* Key Takeaways - TL;DR Section */}
      {post.key_takeaways && post.key_takeaways.length > 0 && (
        <div className="mb-12 p-8 bg-gradient-to-br from-breezy-50 via-white to-sand-50 rounded-3xl border border-breezy-100 shadow-sm">
          <h2 className="text-xl font-bold text-breezy-800 mb-6 flex items-center gap-3">
            <span className="w-10 h-10 rounded-2xl bg-gradient-to-br from-breezy-500 to-breezy-600 flex items-center justify-center text-white shadow-lg shadow-breezy-500/25">
          ...
          <ul className="space-y-4">
            {post.key_takeaways.map((takeaway, idx) => (
              <li key={idx} className="flex items-start gap-4 text-sand-700">
                <span className="w-7 h-7 rounded-xl bg-breezy-100 flex-shrink-0 flex items-center justify-center text-sm font-bold text-breezy-700 mt-0.5">
                  {idx + 1}
                </span>
      ...
      <div className="prose prose-lg max-w-none
        ...
        prose-table:my-8 prose-table:w-full prose-table:rounded-2xl prose-table:overflow-hidden prose-table:shadow-sm prose-table:border prose-table:border-sand-200
        prose-thead:bg-gradient-to-r prose-thead:from-sand-100 prose-thead:to-sand-50
        prose-th:px-5 prose-th:py-4 prose-th:text-left prose-th:font-semibold prose-th:text-sand-900 prose-th:text-sm prose-th:uppercase prose-th:tracking-wide
        prose-td:px-5 prose-td:py-4 prose-td:text-sand-700 prose-td:border-t prose-td:border-sand-100
        prose-tr:transition-colors hover:prose-tr:bg-sand-50/50
