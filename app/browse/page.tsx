...
export default async function BrowsePage() {
  try {
    ...
    if (researchError) {
      console.error('Supabase query error:', researchError);
      return (
        <div className="min-h-screen bg-red-50 flex items-center justify-center">
          <div className="text-center p-8">
            <h1 className="text-2xl font-bold text-red-600 mb-4">Database Query Error</h1>
            <p className="text-red-500">{researchError.message}</p>
            <pre className="mt-4 text-left bg-red-100 p-4 rounded text-sm overflow-auto">
              ...
              <div className="space-y-4">
                {typedResearch
                  ...
                  .map((item) => {
                    ...
                    return (
                      <ResearchCard key={item.id} research={item as ResearchRow & { topic: { name: string; slug: string } }} locked={locked} />
                    );
                  })}
              </div>
            ) : (
